...
def create_session_block_event(session_id, session_name, block_start, block_end, block_num):
    ...
    if num_days == 1:
        summary = f"{session_name} - In Session"
    else:
        summary = f"{session_name} - In Session ({num_days} days)"
...
def generate_calendar(sessions_to_include, output_filename):
    ...
    cal.add('x-wr-calname', 'Legislative Sessions Calendar 2026')
...
def main():
    ...
    print("Legislative Calendar Generator - 2026 Edition")
    ...
    for state in all_states:
        state_name = LEGISLATIVE_SESSIONS_2026[state]['name']
        print(f"{state_name}:")
        generate_calendar(
            [state],
    ...
    print()
    print("Next steps:")
    print("  1. git add output/*.ics")
    print("  2. git commit -m 'Add all 50 states + regional calendars'")
    print("  3. git push")
